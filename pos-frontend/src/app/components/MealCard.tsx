import React from 'react'
import all from '@/app/images/all.png'
import { Plus } from 'lucide-react'
import { Minus } from 'lucide-react'
import Image from 'next/image'

const MealCard = (props:any) => {
  return (
    <div className='bg-white shadow-xl w-[95%] mt-8 rounded-lg'>
                        <div className='flex flex-col gap-0'>
                            <Image src={all} alt='product image' className='rounded-md' />
                            <h1 className='font-semibold text-xl px-4 pt-5'>{props.info.name}</h1>
                            <p className='text-gray-500 text-sm px-4 pt-2'>{props.info.category.name}</p>
                            <div className='flex items-center'>
                                <h3 className='px-4 pt-4 text-xl font-bold mb-5'>{`Rs ${props.info.price}`}</h3>
                                <div className='border-2 border-gray-100 gap-5 flex  px-2 py-2 rounded-full'>
                                    <Minus width={15} />
                                    <p>0</p>
                                    <Plus width={10} color='white' className='bg-red-800 rounded-full w-6' />
                                </div>
                            </div>
                        </div>
                    </div> 
  )
}

export default MealCard
